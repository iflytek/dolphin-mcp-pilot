---
title: Clone a recovery point before moving a live schedule to weekday 07:00
author: whyiug
date: 2026-08-17
...
# 一句话安全改调度：先留恢复点，再验收到真实 SUCCESS

## The task

我需要把 `daily_ops_report` 从每天 06:00 改为工作日 07:00，又不希望一次错误的自然语言操作留下不可逆影响。验收条件不是“命令已提交”，而是：变更前存在一个离线恢复副本、调度更新后保持在线、原工作流和三项任务都到达 `SUCCESS`。
...
2. `ds_clone_workflow(auto_online=false)` 创建恢复点；`ds_list_workflows` 再次确认其为 `OFFLINE`。
...
![The original workflow is online and its recovery copy is offline](workflows.png)
...
| Check | Result |
| --- | --- |
| Recovery workflow | `daily_ops_report_recovery_20260817`, `OFFLINE`, 3 tasks |
...
这次节省的不是几次页面点击，而是把操作顺序变成了可检查的安全协议：先创建恢复点，再执行状态变更，最后同时验证任务终态和流程终态。即使调度修改失败，原工作流也有离线副本；即使触发接口返回 `submitted`，智能体也不会把排队成功误报成执行成功。
...
- 所有内容均来自本地合成实例；认证头从未进入提交材料，私网地址和运行路径已由渲染脚本替换。
